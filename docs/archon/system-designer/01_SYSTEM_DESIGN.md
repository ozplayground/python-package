...
## 2. 6대 모듈 상세 패키지 구조 (Package Architecture)
...
### 2.2 6대 모듈별 상세 역할 및 책임 명세

| 모듈 경로 | 핵심 클래스 / 인터페이스 | 상세 역할 및 구현 엔지니어링 명세 |
| :--- | :--- | :--- |
| `archon/core/` | `Agent`<br/>`AgentSession`<br/>`ExecutionContext`<br/>`StepResult`<br/>`EventLoop` | • **세션 라이프사이클 제어**: 세션 생성, 타스크 실행, 리소스 정리(`close`) 관리.<br/>• **불변 컨텍스트**: 각 실행 턴마다 읽기 전용 `ExecutionContext` 스냅샷 주입.<br/>• **턴 실행 루프**: LLM 추론 $\rightarrow$ 툴 호출 파싱 $\rightarrow$ 툴 실행 $\rightarrow$ 결과 피드백 루프 구동. |
| `archon/harness/` | `HarnessProvider`<br/>`FileSystemHarnessProvider`<br/>`InMemoryHarnessProvider`<br/>`DatabaseHarnessProvider`<br/>`HarnessParser`<br/>`HarnessManifest` | • **다중 소스 지원**: 로컬 FS, Zip 업로드 바이너리(ZipSlip 경로 탈출 방어), DB 테이블에서 하네스 수집.<br/>• **마크다운/YAML 정규화**: `AGENTS.md` 및 `.agents/` 내 프론트매터를 Pydantic 기반으로 파싱 및 검증.<br/>• **불변 매니페스트**: 컴파일된 규칙, 스킬, 서브에이전트 정의를 `HarnessManifest`로 동결(Freeze). |
| `archon/subagents/` | `SubagentDefinition`<br/>`SubagentDispatcher`<br/>`SubagentRunner`<br/>`MessageBus` | • **비동기 병렬 호출**: `invoke_subagents`를 통한 복수 서브에이전트 동시 실행 (`asyncio.gather`).<br/>• **거버넌스 한도 제어**: 최대 재귀 깊이(`max_depth=3`) 제한, 순환 호출 체인 탐지 및 차단.<br/>• **동시성 세마포어**: 단일 세션 내 동시 실행 서브에이전트 수를 제어하여 리소스 고갈 방지. |
| `archon/tools/` | `BaseTool`<br/>`BashTool`<br/>`ToolRegistry`<br/>`@tool`<br/>`ToolExecutionResult` | • **도구 레지스트리**: 함수 기반 `@tool` 데코레이터를 통한 JSON 스키마 자동 추출 및 세션 격리 등록.<br/>• **보안 Bash 샌드박스**: 지정된 작업 디렉토리 감금(Jail), 상위 탈출 차단, 파괴적 명령어 블랙리스트 정규식 검증.<br/>• **프로세스 안전 회수**: `os.setsid` 기반 프로세스 그룹 분리 및 타임아웃 시 `SIGKILL` 전파. |
| `archon/skills/` | `SkillDefinition`<br/>`SkillRegistry`<br/>`SkillLoader` | • **스킬 독립성 강제**: 스킬 본문 내 타 스킬 상호 참조 및 결합을 정적 린터로 탐지하여 위반 시 로드 거부.<br/>• **점진적 온디맨드 주입**: 서브에이전트가 선언한 필수 스킬만 프롬프트에 주입하여 토큰 낭비 방지. |
| `archon/models/` | `BaseModelAdapter`<br/>`MockModelAdapter`<br/>`OpenAIAdapter`<br/>`LiteLLMAdapter` | • **멀티 벤더 추상화**: OpenAI, Anthropic, Gemini, 로컬 vLLM 등 통일된 스트리밍 및 툴 호출 규격 지원.<br/>• **단위 테스트 격리**: 네트워크 비용 없는 결정론적 시나리오 테스트용 `MockModelAdapter` 기본 탑재. |
...
## 4. 3대 핵심 시퀀스 다이어그램 (Sequence Diagrams)
...
sequenceDiagram
    ...
    Dispatcher->>Dispatcher: 재귀 깊이 검사 (depth < 3) 및 순환 호출 검사
    
    par 백엔드 서브에이전트 실행
        Dispatcher->>Sem: 세마포어 획득 대기
...
sequenceDiagram
    ...
    alt 상위 탈출(cd ../..) 또는 블랙리스트(rm -rf /, sudo 등) 감지 시
        ...
        alt 제한 시간(timeout) 내 정상 종료
            ...
            Note over Bash, OS: 타임아웃 감지 -> 프로세스 그룹 전체 강제 종료
...
### 5.1 서브에이전트 재귀 호출 깊이 및 순환 탐지 정책
...
def create_session(
    ...
    *,
    ...
    """
    하네스 소스를 로드하고 검증하여 격리된 AgentSession 인스턴스를 생성합니다.
...
class AgentSession:
    ...
    def run(
        ...
        *,
        ...
        """
        메인 에이전트를 동기적으로 실행하고 최종 완료 결과를 반환합니다.
        """
        ...
    ...
    async def stream(
        ...
        *,
        ...
        """
        메인 에이전트의 생성 텍스트 토큰을 실시간 비동기 제너레이터로 스트리밍합니다.
        """
        ...
    ...
    async def invoke_subagents(
        ...
        """
        복수의 서브에이전트를 비동기 병렬로 동시 실행하고 결과를 취합하여 반환합니다.
...
def calculate_vat(amount: int, rate: float = 0.1) -> int:
    """
    파이썬 타입 힌트를 기반으로 JSON Schema를 자동 추출하여 LLM 도구 호출 규격으로 변환합니다.
    """
...
2. 공개 클래스 및 인터페이스 목록 (Public Classes & Interfaces)
3. 타 모듈과의 의존성 제약 (Dependency Rules - 상향 의존 금지)
4. 단위 테스트 실행 가이드 및 주요 엣지 케이스 (Testing Guide)
```

- **`archon/core/README.md`**: 세션 라이프사이클 관리 및 불변 `ExecutionContext` 보존 원칙, 턴 루프 인터페이스 기술.
- **`archon/harness/README.md`**: FS/Upload/DB 프로바이더 확장법, ZipSlip 방어 원칙, `HarnessManifest` 스냅샷 스키마 기술.
- **`archon/subagents/README.md`**: 병렬 디스패치 원리, 재귀 깊이(`max_depth=3`) 및 순환 탐지 알고리즘, 세마포어 정책 기술.
- **`archon/tools/README.md`**: 커스텀 툴 작성법, BashTool 보안 정규식 블랙리스트 및 `os.setsid` 프로세스 그룹 격리 정책 기술.
- **`archon/skills/README.md`**: 스킬 독립성 원칙, 정적 린터 검증 규칙 및 온디맨드 프롬프트 주입 메커니즘 기술.
- **`archon/models/README.md`**: 멀티 LLM 어댑터 인터페이스 확장법, 단위 테스트 시 `MockModelAdapter` 활용법 기술.
